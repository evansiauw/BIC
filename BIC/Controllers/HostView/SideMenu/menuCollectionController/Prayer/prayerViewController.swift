...
class prayerViewController: UIViewController {
    ...
    func successAlert(){
        ...
        // Generate top floating entry and set some properties
        var attributes = EKAttributes.bottomFloat
        attributes.entryBackground = .gradient(gradient: .init(colors: [UIColor.blue, UIColor.green], startPoint: .zero, endPoint: CGPoint(x: 1, y: 1)))
        attributes.popBehavior = .animated(animation: .init(translate: .init(duration: 0.3), scale: .init(from: 1, to: 0.7, duration: 0.7)))
        attributes.shadow = .active(with: .init(color: .black, opacity: 0.5, radius: 10, offset: .zero))
        attributes.statusBar = .dark
        attributes.scroll = .enabled(swipeable: true, pullbackAnimation: .jolt)
        ...
        let title = EKProperty.LabelContent(text: "Success!!!", style: .init(font: UIFont.systemFont(ofSize: 16, weight: .heavy), color: UIColor.white))
        let description = EKProperty.LabelContent(text: "Mark 11:24 Therefore I tell you, whatever you ask in prayer, believe that you have received it, and it will be yours.", style: .init(font: UIFont.systemFont(ofSize: 13, weight: .bold), color: UIColor.white))
